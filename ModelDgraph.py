...
def create_data(client):
    # Cargar usuarios
    users_uids = load_Users('data/User.csv')
    print(f"Usuarios cargados: {users_uids}")

    # Cargar productos
    productos_uids = load_productos('data/productos.csv')
    print(f"Productos cargados: {productos_uids}")

    # Cargar categorías
    categorias_uids = load_categoria('data/categorias.csv')
    print(f"Categorías cargadas: {categorias_uids}")

    # Cargar devoluciones
    devoluciones_uids = load_devolucion('data/devoluciones.csv')
...
# Query quitar de favoritos 
def quitar_de_favoritos(client, username, productoNombre):
    # 1. Buscar el UID del producto por nombre
    query_producto = f"""
    {{
        search_producto(func: eq(nombre, "{productoNombre}")) @filter(type(Producto)) {{
    ...
    # 3. Quitar el producto de los favoritos del usuario
    txn = client.txn()
    try:
        mutation = {
            'uid': user_uid,
        ...
        txn.mutate(del_obj=mutation, commit_now=True)
        print(f"Producto '{productoNombre}' eliminado de los favoritos del usuario '{username}'.")
    finally:
        txn.discard()

def actualizar_username(client, actual_username, nuevo_username):
    # Buscar usuario por actual_username
    query = f"""
    {{
        user(func: eq(username, "{actual_username}")) @filter(type(User)) {{
            uid
        }}
    }}
    """
    ...
    try:
        ...
        user = data.get("user", [])
        if not user:
            print(f"Usuario '{actual_username}' no encontrado.")
            return
        uid = user[0]["uid"]
    finally:
        txn.discard()
    ...
    query_check = f"""
    {{
        existing_user(func: eq(username, "{nuevo_username}")) @filter(type(User)) {{
            uid
        }}
    }}
    """
    ...
    try:
        ...
        txn.mutate(set_obj=mutation, commit_now=True)
        print(f"Username actualizado de '{actual_username}' a '{nuevo_username}'.")
    finally:
        txn.discard()

def actualizar_telefono(client, username, nuevo_telefono):
    # 1. Buscar el UID del usuario con el nombre de usuario
    query = f"""
    {{
        user(func: eq(username, "{username}")) @filter(type(User)) {{
            uid
    ...
    # 2. Ejecutar la mutación para actualizar el teléfono
    txn = client.txn()
    try:
        mutation = {
            "uid": uid,
        ...
        txn.mutate(set_obj=mutation, commit_now=True) 
        print(f"Teléfono del usuario '{username}' actualizado a '{nuevo_telefono}'.")
    finally:
        txn.discard()
